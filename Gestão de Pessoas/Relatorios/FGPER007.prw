...
Static Function printPage()
	...
	IF DbSeek(xFilial("SRA")+mv_par01)
	   ...
	   oPrinter:Say(n_Lin+= 50, n_Col  ,"Declaramos para fins exclusivos de abertura de conta salário, que Sr(a): ",oArial10N,,0)
	   oPrinter:Say(n_Lin+= 50, n_Col  ,""+  c_Nome + ', CTPS: ' + alltrim(c_Ctps) ,oArial10N,,0)
	   oPrinter:Say(n_Lin+= 50, n_Col  ,""+  'RG n°:  '+  alltrim(c_Registro) +' , '+ 'CPF n°: '+ c_CPF + ' residente : ' ,oArial10N,,0)
	   oPrinter:Say(n_Lin+= 50, n_Col  ,""+ c_End +', é nosso funcionário',oArial10N,,0)
  	   oPrinter:Say(n_Lin+= 50, n_Col  ,"desde " +  dtoc(d_Admissa) +' com salário mensal de: ' + alltrim(cvaltochar(n_Salario)) +' '+ c_SalExt +'.',oArial10N,,0)
