...
Static Function f_ImportaDados()
	...
	IF AVISO(SM0->M0_NOMECOM,"Esta rotina irá importar os ativos a partir do arquivo CSV selecionado  pelo usuário para o Cadastro de Ativos do sistema. Deseja realmente continuar?",{"Sim","Não"},2,"Atenção") == 1
		IF !l_CriaTb
		...
		WHILE !FT_FEOF()
			...
			//GRUPO 	CÓD. BEM 	AQUISIÇÃO 	STATUS 	N. FISCAL 	ITEM 	DESCRIÇÃO 	CHASSIS	PLACA 	QUANT. 	VALOR 	CÓD. FORNEC.	LOJA 	VIDA ÚTIL 	MESES USO 	PLAQUETA
			//04 		10 			10 			10 		9 			2 		140 		20 		8 		3 		10 		6 				2		3 			3 			8
			//NÚMERO 	NÚMERO 	    DATA 		TEXTO 	NÚMERO 		NÚMERO 	TEXTO 		TEXTO 	TEXTO 	NÚMERO 	NÚMERO 	TEXTO 			TEXTO	NÚMERO 		NÚMERO 		ALFANUMÉRICO

			//VENDA 	DEPRECIA 	CATEGORIA 	MARCA	UF		N.C.M. 	CNPJ    VIDA ÚTIL	MESES USO
			//10		10 			10 			20 		2 		10 		18 		3 			3 	
			//DATA 		DATA 		TEXTO 		TEXTO 	TEXTO 	NÚMERO 	NÚMERO 	NÚMERO 		NÚMERO

			c_GRUPO   := PADR(UPPER(AllTrim(a_Buffer[1])), TAMSX3("N1_GRUPO")[1])	  		// Grupo do Bem
			c_CBASE   := PADR(UPPER(AllTrim(a_Buffer[2])), TAMSX3("N1_CBASE")[1])  		    // Código do Bem
			c_ITEM    := PADR(UPPER(AllTrim(a_Buffer[3])), TAMSX3("N1_ITEM")[1])	  		// Item do Bem
			c_DESCRIC := PADR(UPPER(AllTrim(a_Buffer[4])), TAMSX3("N1_DESCRIC")[1])	  		// Descrição do Bem
			c_PATRIM  := PADR(UPPER(AllTrim(a_Buffer[5])), TAMSX3("N1_PATRIM")[1])	  		// Classificação do Bem
			c_AQUISIC := PADR(UPPER(AllTrim(a_Buffer[6])), TAMSX3("N1_AQUISIC")[1])	  		// Data de Aquisição
			...
			c_CALCPIS := PADR(UPPER(AllTrim(a_Buffer[9])), TAMSX3("N1_CALCPIS")[1])	  		// Cálcula PIS/COFINS
			c_APOLICE := PADR(UPPER(AllTrim(a_Buffer[10])), TAMSX3("N1_APOLICE")[1])  		// Apólice
			c_DTVENC  := PADR(UPPER(AllTrim(a_Buffer[11])), TAMSX3("N1_DTVENC")[1])	  		// Data de Vencimento da Apólice
			c_CODSEG  := PADR(UPPER(AllTrim(a_Buffer[12])), TAMSX3("N1_CODSEG")[1])	  		// Código da Seguradora
			...
			c_NSERIE  := PADR(UPPER(AllTrim(a_Buffer[17])), TAMSX3("N1_NSERIE")[1])	  		// Série da NF
			...
			c_CODCIAP := PADR(UPPER(AllTrim(a_Buffer[21])), TAMSX3("N1_CODCIAP")[1])  		// Código CIAP
			c_ICMSAPR := PADR(UPPER(AllTrim(a_Buffer[22])), TAMSX3("N1_ICMSAPR")[1])  		// ICMS do Item
			c_CODBAR  := PADR(UPPER(AllTrim(a_Buffer[23])), TAMSX3("N1_CODBAR")[1])	  		// Código de Barra
			c_PENHORA := PADR(UPPER(AllTrim(a_Buffer[24])), TAMSX3("N1_PENHORA")[1])  		// Situação de Penhora
			c_VLAQUIS := PADR(UPPER(AllTrim(a_Buffer[25])), TAMSX3("N1_VLAQUIS")[1])  		// Valor de Aquisição
			...
			c_CSTPIS  := PADR(UPPER(AllTrim(a_Buffer[28])), TAMSX3("N1_CSTPIS")[1])	  		// Situação Tributária PIS
			c_ALIQPIS := PADR(UPPER(AllTrim(a_Buffer[29])), TAMSX3("N1_ALIQPIS")[1])  		// Alíquota PIS
			c_CSTCOFI := PADR(UPPER(AllTrim(a_Buffer[30])), TAMSX3("N1_CSTCOFI")[1])  		// Situação Tributária COFINS
			c_ALIQCOF := PADR(UPPER(AllTrim(a_Buffer[31])), TAMSX3("N1_ALIQCOF")[1])  		// Alíquota COFINS
			c_CODBCC  := PADR(UPPER(AllTrim(a_Buffer[32])), TAMSX3("N1_CODBCC")[1])	  		// Código de Base de Cálculo do Crédito
			c_NUMPRO  := PADR(UPPER(AllTrim(a_Buffer[33])), TAMSX3("N1_NUMPRO")[1])	  		// Tipo de Processo Referenciado
			c_CBCPIS  := PADR(UPPER(AllTrim(a_Buffer[34])), TAMSX3("N1_CBCPIS")[1])	  		// Base de Cálculo do PIS/COFINS
			c_OBS     := ""
	...
	ENDIF
				...
				If Upper(AllTrim(c_Ncm)) == "BLOQUEAR"
					...
					c_Obs  := "Produto " + AllTrim(c_CBase) + " foi bloqueado pela rotina, porque o Pos.IPI/NCM cadastrado é inválido."
				Else
					RECLOCK("SB1", .F.)
					SB1->B1_POSIPI := c_Ncm
					MSUNLOCK()
		...
		FT_FUSE()
		AVISO(SM0->M0_NOME,"O programa processou todo o arquivo com " + ALLTRIM(STR(n_QtdInc+n_QtdErr)) + " registros. Foram atualizados " + ALLTRIM(STR(n_QtdInc)) + " registros e " + ALLTRIM(STR(n_QtdErr)) + " registros não foram atualizados pela rotina.",{"OK"},2,"Aviso")
		...
		o_Dlg:= MSDialog():New( 091,232,637,1240,"Log de Atualização de Pos.IPI/NCM da tabela SB1 - Cadastro de Produtos",,,.F.,,,,,,.T.,,,.T. )
