...
While !SA1->(EOF()) .And. SA1->A1_FILIAL == xFilial("SA1")
	...
	If !(CTH->(DbSeek(xfilial("CTH")+"C"+ALLTRIM(SA1->A1_COD)+ALLTRIM(SA1->A1_LOJA))))
		...
		oProcess:IncRegua2("Cliente: " + "C"+ALLTRIM(SA1->A1_COD)+ALLTRIM(SA1->A1_LOJA)+ " - " + AllTrim(SA1->A1_NOME))
		If SA1->A1_MSBLQL <> "1"
			RecLock("CTH",.T.)
...
While !SA2->(EOF())  .And. SA2->A2_FILIAL == xFilial("SA2")
	...
	If !(CTH->(DbSeek(xFilial("CTH")+"F"+ALLTRIM(SA2->A2_COD)+ALLTRIM(SA2->A2_LOJA))))
		...
		oProcess:IncRegua2("Fornecedor: " + "F"+ALLTRIM(SA2->A2_COD)+ALLTRIM(SA2->A2_LOJA)+ " - " + AllTrim(SA2->A2_NOME))
		If SA2->A2_MSBLQL <> "1"
			RecLock("CTH",.T.)
			CTH->CTH_FILIAL	:= xFilial("CTH")
			...
			CTH->CTH_BLOQ	:= "2"
			CTH->CTH_YCGC   := SA2->A2_CGC  //- CASO O FORNCEDOR QUEIRA COLOCAR UM FILTRO NA CLASSE DE VALOR POR CNPJ
