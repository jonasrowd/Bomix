...
User Function BLTITAU()
	Local lOK      := .T. // Controle de impressão da rotina
	Private cAlias := ""  // Alias da tabela temporária

	// Cria o Pergunte()
	FCriaSX1()

	// Verifica se a execução está sendo realizada via
	// interface gráfica, caso sim, abre a tela do Pergunte()
	If (!IsBlind())
	...
	// Se o usuário clicou em OK ou o fonte está sendo
	// excutado via job, monta a tabela temporária
	If (lOK)
		cAlias := TableDef()
	EndIf
	...
	// Se via interface gráfica, também imprime
	If (lOK)
		Processa({||MontaRel()})
	EndIf
Return (NIL)
...
Static Function MontaRel()
	...
	Do While (cAlias)->(!EOF())
	   ...
	   IF  (cAlias)->E1_PORTADO == "CX1"
	       (cAlias)->(DBSKIP())
	       LOOP
	   ELSE
	       DBSELECTAREA("SC5")
	       SC5->(DBSETORDER(1))
	       IF SC5->(DBSEEK(XFILIAL("SC5") + (cAlias)->E1_PEDIDO))
				IF SC5->C5_BANCO == "CX1"
					(cAlias)->(DBSKIP())
					LOOP
				ENDIF
	       ...
	       IF SF2->(DBSEEK(XFILIAL("SF2") + (cAlias)->(E1_NUM + E1_SERIE + E1_CLIENTE + E1_LOJA)))
			   ...
			   IF SE4->(DbSeek(xFilial("SE4") + CCONDPAG))
				  IF (SE4->E4_TIPO == "1") .OR. (SE4->E4_TIPO == "5")
				  		...
				  		IF (VAL(APAGAMENTOS[1]) == 0) .AND. (VAL((cAlias)->E1_PARCELA) <= 1)
				             (cAlias)->(DBSKIP())
				             LOOP
				  		ENDIF
	   ...
	   ENDIF
		...
		If Marked("E1_OK")
			If Empty(MV_PAR19)
				...
				DbSetOrder(1)
				//DbSeek(xFilial("SA6")+MV_PAR19+MV_PAR20+MV_PAR21)
				If DbSeek(xFilial("SA6")+(cAlias)->(E1_PORTADO+E1_AGEDEP+E1_CONTA)) == .F.
						Help(NIL, NIL, "CADASTRO BANCO", NIL, ("Banco " + MV_PAR19 + "/ Agência: " + MV_PAR20 + " /Conta: " + MV_PAR21 + " inválidos.");
				...
				If DbSeek(xFilial("SEE")+(cAlias)->(E1_PORTADO+E1_AGEDEP+E1_CONTA)) == .F.
						Help(NIL, NIL, "CADASTRO BANCO", NIL, ("Banco " + MV_PAR19 + "/ Agência: " + MV_PAR20 + " /Conta: " + MV_PAR21 + " inválidos.");
						,1, 0, NIL, NIL, NIL, NIL, NIL, {"Verifique se os parâmetros estão preenchidos corretamente."})
					Return
				Endif
			Else
				...
				If DbSeek(xFilial("SEE")+MV_PAR19+MV_PAR20+MV_PAR21) == .F.
					Help(NIL, NIL, "CADASTRO BANCO", NIL, ("Parâmetros de Banco " + MV_PAR19 + "/ Agência: " + MV_PAR20 + " /Conta: " + MV_PAR21 + " inválidos.");
					,1, 0, NIL, NIL, NIL, NIL, NIL, {"Verifique se os parâmetros estão preenchidos corretamente."})
					Return
				Endif
			Endif
			...
			DbSelectArea("SC5")
			DbSetOrder(1)
			DbSeek(xFilial("SC5")+(cAlias)->E1_PEDIDO,.T.)

			If !empty(SC5->C5_CLIENTE+SC5->C5_LOJACLI).and.(SC5->C5_FILIAL+SC5->C5_NUM==(cAlias)->E1_FILIAL+(cAlias)->E1_PEDIDO)
				DbSelectArea("SA1")
				DbSetOrder(1)
			...
			Else
				...
				DbSelectArea("SA1")
				DbSetOrder(1)
				DbSeek(xFilial("SA1")+(cAlias)->E1_CLIENTE+(cAlias)->E1_LOJA,.T.)
			Endif
			...
			//nVlrAbat   :=  SomaAbat((cAlias)->E1_PREFIXO,(cAlias)->E1_NUM,(cAlias)->E1_PARCELA,"R",1,,(cAlias)->E1_CLIENTE,(cAlias)->E1_LOJA)
			/*
			--------------------------------------------------------------
			Parte do Nosso Numero. Sao 8 digitos para identificar o titulo
			--------------------------------------------------------------
			*/
			cNroDoc	:= StrZero(	Val(Alltrim((cAlias)->E1_NUM)+Alltrim((cAlias)->E1_PARCELA)),8)
			DbSelectArea("SE1")
			RecLock("SE1",.F.)
			(cAlias)->E1_NUMBCO 	:=	cNroDoc   		// Nosso número (Ver fórmula para calculo)
			(cAlias)->E1_PORTADO :=	aDadosBanco[1]
			(cAlias)->E1_AGEDEP  :=	aDadosBanco[3]
			(cAlias)->E1_CONTA   :=	aDadosBanco[4]
			MsUnlock()
			/*
			----------------------
			...
			*/
			...
			Elseif  (cAlias)->E1_SALDO == 0 .And. Empty((cAlias)->E1_PARCELA)
	       ...
	       // MsgBox(STR(nVlrAbat))
			...
			oPrint:cPathPDF := cLocal //Acrescentado destino na variável
...
Static Function Impress(oPrint,aDadosEmp,aDadosTit,aDadosBanco,aDatSacado,aBolText,aCB_RN_NN,dvNN)
	...
	*/
	...
	//Código de barras utilizando FWMsBar()
	oPrint:FwMsBar("INT25",65.5,2,StrTran(StrTran(aCB_RN_NN[2], ".", ""), " ", ""),oPrint,.F.,Nil,Nil,0.0225,1,Nil,Nil,"A",.F.)
	//DbSelectArea("SE1")
		//RecLock("SE1",.f.)
		//(cAlias)->E1_NUMBCO 	:=	aCB_RN_NN[3]   // Nosso número (Ver fórmula para calculo)
	//MsUnlock()
...
/*/{Protheus.doc} FCriaSX1
	Função para criação de registros na SX1
	@type Function
	@version 12.1.25
...
Static Function FCriaSX1()
	Local oSX1 := CreateSX1():New("BLTITAU") // Cria instância do objeto
...
/*/{Protheus.doc} TableDef
	Monta a tabela temporária que será usada no boleto
	@type Function
	@version 12.1.25
	@author Jonas Machado
	@since 09/07/2021
	@return Character, Alias da tabela temporária
/*/
Static Function TableDef()
	...
	// Adiciona um índice e cria a tabela
...
/*/{Protheus.doc} QueryDef
	Monta o comando SQL para montagem da tabela temporária
	@type Function
	@version 12.1.25
	@author Jonas Machado
	@since 09/07/2021
...
Static Function QueryDef()
	...
	// Monta a cláusula IN de MVABATIM
	...
	// Caso não, usa o portador "de até"
	If (!Empty(MV_PAR19))
		cQuery += "AND "
	...
	// Se a agência for preenchida, adiciona na query
	If (!Empty(MV_PAR20))
		cQuery += "AND "
...
Method NewItem(aList) Class CreateSX1
	Local nX      := 0                 // Contador do laço
	Local aPos    := {}                // Vetor de posições da estrutura da SX1
	Local aItem   := {}                // Vetor auxiliar para criação da nova linha
	...
	// Preencher com os dados enviados pelo usuário
	...
	// Adiciona a linha criada no total de registro e limpa a memória
...
Method Commit() Class CreateSX1
	Local nX     := 0         // Contador do laço de registros da SX1
	Local nY     := 0         // Contador do laço de campos de cada registro da SX1
	Local aArea  := GetArea() // Tabela posicionada anteriormente
	Local lFound := .F.       // Verificação de existência de registro

	// Isola a transação para manter a atomicidade
	BEGIN TRANSACTION
		// Percorre as linhas adicionadas no objeto
		For nX := 1 To Len(::aSX1)
			// Posiciona na SX1 e verifica se o registo já existe
			DBSelectArea("SX1")
			DBSeek(::cGroup + ::aSX1[nX][2])
			lFound := Found()

			// Se o grupo de perguntas já existir, altera, se não, cria
			RecLock("SX1", !lFound)
