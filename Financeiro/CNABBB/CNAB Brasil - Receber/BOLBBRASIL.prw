...
ฑฑณPrograma  ณ BOLBBRASIL												  ณฑฑ
ฑฑรฤฤฤฤฤฤฤฤฤฤลฤฤฤฤฤฤฤฤฤฤมฤฤฤฤฤฤฤมฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤมฤฤฤฤฤฤมฤฤฤฤฤฤฤฤฤฤดฑฑ
ฑฑณDescri…o ณ IMPRESSAO DO BOLETO BANCO BRASIL COM CODIGO DE BARRAS      ณฑฑ
...
LOCAL aDadosEmp    := {	ALLTRIM(SM0->M0_NOMECOM)                                    ,; //[1]Nome da Empresa
								ALLTRIM(SM0->M0_ENDCOB)                                     ,; //[2]Endere็o
...
ProcRegua(RecCount())
Do While SE1->(!EOF())
	If Marked("E1_OK")  // Imprime apenas os que foram marcado para impressใo 
		If Empty(MV_PAR01)
			...
			If DbSeek(xFilial("SA6")+SE1->(E1_PORTADO+E1_AGEDEP+E1_CONTA)) == .F.
				ShowHelpDlg(SM0->M0_NOME,;
					{"Banco/Ag๊ncia/Conta invแlido."},5,;
					{"Verifique se os parโmetros estใo preenchidos corretamente."},5)
				Return
			Endif
			...
			If DbSeek(xFilial("SEE")+SE1->(E1_PORTADO+E1_AGEDEP+E1_CONTA)+MV_PAR04) == .F.
				ShowHelpDlg(SM0->M0_NOME,;
					{"Parโmetros de Banco do Banco/Ag๊ncia/Conta/Sub-Conta invแlido."},5,;
					{"Verifique se os parโmetros estใo preenchidos corretamente."},5)
				Return
			Endif
		Else
	...
	/*
		aDadosBanco  := {  "001" ,;                   //SA6->A6_COD 					// [1]Numero do Banco
	                      ...
	                      Alltrim(SA6->A6_DVCTA),; 	// 								// [5]Dํgito da conta corrente
...
oPrint:Say  (nRow2+0750,100 ,"PAGมVEL EM QUALQUER BANCO ATษ O VENCIMENTO",oFont10)
...
oPrint:Say  (nRow2+0910,1005,"Esp้cie Doc."                                   ,oFont8)
...
oPrint:Say  (nRow2+0980,755 ,"Esp้cie"                                        ,oFont8)
...
oPrint:Say  (nRow2+1050,100 ,"Instru็๕es (Todas informa็๕es deste bloqueto sใo de exclusiva responsabilidade do cedente)",oFont8)
...
oPrint:Say  (nRow2+1645,1500,"Autentica็ใo Mecโnica",oFont8)
...
oPrint:Say  (nRow3+2040,100 ,"PAGมVEL EM QUALQUER BANCO ATษ O VENCIMENTO",oFont10)
...
oPrint:Say  (nRow3+2200,1005,"Esp้cie Doc."                                   ,oFont8)
...
oPrint:Say  (nRow3+2340,100 ,"Instru็๕es (Todas informa็๕es deste bloqueto sใo de exclusiva responsabilidade do cedente)",oFont8)
...
oPrint:Say  (nRow3+2410,1810,"(-)Outras Dedu็๕es"                             ,oFont8)
...
oPrint:Say  (nRow3+2855,1500,"Autentica็ใo Mecโnica - Ficha de Compensa็ใo"                        ,oFont8)
...
If Empty(SE1->E1_NUMBCO) .or. MV_PAR11 = 1	
	If Len(AllTrim(SEE->EE_CODEMP)) == 6		//Conv๊nio de 6 posi็๕es
		NNUM      := Strzero(Val(SEE->EE_FAXATU),5)
	  	cNosso    := Strzero(Val(SEE->EE_CODEMP),6)+NNUM
	  	cDigNosso := U_CALC_di9(cNosso)
	Elseif Len(AllTrim(SEE->EE_CODEMP)) == 7	//Conv๊nio de 7 posi็๕es
...
// Pelo pessoal da Microsiga nใo existia a linha abaixo
if sumdig<10
  nDezena=10
else 
  nDezena:=VAL(ALLTRIM(STR(VAL(SUBSTR(cvalor,1,1))+1,12))+"0")
endif
//// Aqui termina as modifica็๕es
