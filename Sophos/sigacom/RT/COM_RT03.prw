...
Local c_Corpo		:= ""
Local c_Para		:= ""
Local c_Assunto		:= "Cotação BOMIX: "+c_NumCot
Local c_Ass1		:= "CONFIRMAÇÃO DE ENVIO COTAÇÃO: "+c_NumCot
Local l_ExibeTela	:= .F.
Local n_Item		:= 1
Local a_Enviado		:= {}
Local _cDescri		:= ""
...
If Aviso(SM0->M0_NOMECOM,"Deseja realmente enviar a cotação "+c_NumCot+" para todos os seus fornecedores que ainda não foram enviados?",{"Sim","Não"},2,"Atenção") == 1
	If Aviso(SM0->M0_NOMECOM,"Deseja anexar algum documento?",{"Sim","Não"},2,"Atenção") == 1
		cArqSaida := REICON01()
		Alert(cArqSaida)
	Else
		cArqSaida := '1'
	...
	While COTEMAIL->( !Eof() )
		...
		c_Corpo += '        	<tr>'
		c_Corpo += '            	<td width = "100%"><font face = "Arial" size = "2">COTAÇÃO : <b>'+c_NumCot+'</b>,</font>'
		...
		c_Corpo += '            <p align="center">	<td width = "45%"><font face = "Arial" size = "2">DESCRIÇÃO</font></td></center>''
		...
		c_Corpo += '        	<tr>'
		c_Corpo += '    		<br>'
		c_Corpo += '            <p><font face = "Arial" size = "2">Informe a condição de pagamento: ______________</font></p>'
		c_Corpo += '    		<br>'
		...
		c_Corpo += '            		<p><font face="arial" size="2">OBS. 1: Nas aquisições de materiais de consumo e ativo imobilizado,quando realizadas fora do Estado da Bahia, o fornecedor será o </font></p>'
		c_Corpo += '            		<p><font face="arial" size="2">responsável pelo recolhimento do imposto decorrente da diferença entre a aliquota interna do Estado da Bahia em relação à aliquota do	 </font></p>'
		c_Corpo += '            		<p><font face="arial" size="2">Estado de Origem, anexando cópia do DAE pago à Nota Fiscal de Fornecimento. </font></p>'
		c_Corpo += '    		<br>'
		c_Corpo += '            		<p><font face="arial" size="2">OBS. 2: Os dados constantes da nota fiscal deverão ser transmitidos através de programa disponibilizado pela secretariada fazenda,no	 </font></p>'
		c_Corpo += '            		<p><font face="arial" size="2">endereço eletronico www.sefaz.ba.gov.br, cujo comprovante de transmissão deverá ser anexado ao documento fiscal(compra legal)</font></p>'
		c_Corpo += '            		<p><font face="arial" size="2">e conforme artigo 228B do ICMS/BA, Decreto 6284 de 14/03/1997</font></p>'
		c_Corpo += '    		<br>'
		c_Corpo += '            		<p><font face="arial" size="2">OBS. 3: É obrigatório informar a classificação fiscal NCM( Nomenclatura Comum do Mercosul ) do produto na Nota Fiscal.	 </font></p>'
		c_Corpo += '    		<br>'
		c_Corpo += '            		<p><font face="arial" size="2">OBS. 4: Condição de pagamento até 08 dias utéis.							 </font></p>'
		c_Corpo += '    		<br>'
		c_Corpo += '           			<p><font face = "Arial" size = "2">Em caso de dúvidas entrar em contato com nosso Seção de Compras.</font></p>'
