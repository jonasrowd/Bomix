...
User Function FMATR700()

Local oReport
Local cDescProd := IIf(cPaisloc == "PTG","Artigo","Produto")

// Adiciona pergunta
PutSx1("MTR700","14","Imprime Descricao?","¿Imprime Descripcion?","Print Description?","mv_che","N",1,0,1,"C","","","","","mv_par14","Pedido","Pedido","Order","",cDescProd,"Producto","Product","","","","","","","","","",;
		{"Define se a impressao da descricao", "dos itens do PV devem imprimir","descricao do PV ou do produto"}, ;
		{"",""} , ;
		{"",""})
...
#IFNDEF TOP
	...
	Local cAliasSF4  := "SF4"
#ELSE	
	Local cAliasSC5 := cAliasSC6 := cAliasSC9 := cAliasSF4 := GetNextAlias()	
#ENDIF
...
Local lImp 		 := .F.
Local cQueryAdd  := ""
Local cFilUsuSC5 := oReport:Section(1):GetAdvplExp("SC5")
Local cFilUsuSC6 := oReport:Section(1):GetAdvplExp("SC6")
        ...
        If mv_par06 == 3
			...
			Else
				...
				WHERE
					SC5.C5_FILIAL = %xFilial:SC5% AND SC5.C5_NUM >= %Exp:mv_par01% AND SC5.C5_NUM <= %Exp:mv_par02% AND
					SC5.%notdel% AND 
					SC5.C5_VEND1 IN %Exp:_TBcFiltro% AND
					SC6.C6_FILIAL = %xFilial:SC6% AND SC6.C6_NUM   = SC5.C5_NUM AND
					SC6.C6_PRODUTO >= %Exp:mv_par03% AND
					SC6.C6_PRODUTO <= %Exp:mv_par04% AND
					SC6.C6_ENTREG  >= %Exp:dtos(mv_par10)% AND
					SC6.C6_ENTREG  <= %Exp:dtos(mv_par11)% AND
...
While !oReport:Cancel() .AND. !( cAliasSC6 )->( Eof() ) .AND. (cAliasSC6)->C6_FILIAL == xFilial("SC6")
	...
	If mv_par06 == 3	//Todos os pedidos
		//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
		//³ Filtra pela tabela SC5.					 ³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		If !Empty(cFilUsuSC5)
			If !(cAliasSC6)->(&cFilUsuSC5)
				(cAliasSC6)->(dbSkip())
				Loop
			Endif
		EndIf
		//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
		//³ Filtra pela tabela SC6.					 ³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		If !Empty(cFilUsuSC6)
			If !(cAliasSC6)->(&cFilUsuSC6)
				(cAliasSC6)->(dbSkip())
				Loop
			Endif
		EndIf
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Fun‡…o    ³ AjustaSX5³ Autor ³ Andre Anjos			³ Data ³18/12/2007³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³ Inclui itens 07 e 08 do C6_OP no SX5 					  ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³ Uso      ³ MATR700			                                          ³±±
±±ÀÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
aAdd(aRegX5,{xFilial(),"E2","07","LIBERADO POR CREDITO E ESTOQUE PENDENTE/BLOQUEADO",;
													"APROBADO POR CREDITO Y STOCK PENDIENTE/BLOQUEADO",;
													"RELEASED BY CREDIT AND STOCK OUTSTANDING/BLOCKED"})

aAdd(aRegX5,{xFilial(),"E2","08","LIBERACAO DE ESTOQUE EFETUADA",;
													"LIBERACION DE STOCK EFECTUADA",;
													"STOCK LIBERATION MADE"})

dbSelectArea("SX5")
If !MsSeek(xFilial()+"E208")
	For nI:=1 to Len(aRegX5)
		RecLock("SX5",.T.)
		REPLACE X5_FILIAL WITH aRegX5[nI][1]
		REPLACE X5_TABELA WITH aRegX5[nI][2]
		REPLACE X5_CHAVE WITH aRegX5[nI][3]
		REPLACE X5_DESCRI WITH aRegX5[nI][4]
		REPLACE X5_DESCSPA WITH aRegX5[nI][5]
		REPLACE X5_DESCENG WITH aRegX5[nI][6]
		MsUnlock()
	Next nI
EndIf
