    ...
    If TcSrvType()<>"AS/400"
		...
		If !Empty(_TBcFiltro)
			...
			SELECT F2_DOC,F2_SERIE,F2_EMISSAO,F2_TIPO,F2_ICMSRET
				...
				,D2_VALIPI,D2_CODISS,D2_VALISS,D2_VALICM,F2_FRETE,F2_SEGURO,F2_DESPESA, D2_GRADE,D2_PEDIDO, D2_ITEMPV, C5_VEND1
			FROM %Table:SF2% SF2, %Table:SD2% SD2
			INNER JOIN %Table:SC5% SC5
				ON	C5_FILIAL = %xFilial:SC5%
	            AND C5_NUM    = D2_PEDIDO
	            AND C5_VEND1 IN %Exp:_TBcFiltro%
				AND SC5.%notdel%              
