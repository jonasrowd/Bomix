...
±±³Programa  ³ FFATR003³ Autor ³ Eduardo José Zanardo  ³ Data ³ 26.12.01 ³±±
...
#IFDEF TOP
	If TcSrvType() <> "AS/400"
		...
		cQuery += "SC6.C6_LOJA,SC6.C6_FSDTSUG,SC6.C6_DESCONT,SC6.C6_LOCAL,"
	...
	If lFiltro
		...
		While !((cAliasSC6)->(Eof())) .And. xFilial("SC6")==(cAliasSC6)->C6_FILIAL .And.;
			...
			aadd(aItemPed,	{	(cAliasSC6)->C6_ITEM,;
				...
				(cAliasSC6)->C6_QTDENT,;
				                      ,;
				(cAliasSC6)->C6_PEDCLI})	//aItemPed[21] - Nº PEDIDO DO CLIENTE
...
@li,123   psay aItemPed[nItem][14]		//C6_FSDTSUG
...
c_Observ := UPPER(Posicione("SC5", 1, xFilial("SC5") + aC5Rodape[1][8], "C5_FSOBS"))
...
IF !(aCabPed[1]$"DB")		//C5_TIPO
	...
	@ 05,125 psay IIF(!Empty(aItemPed[1][21]), "Nº PEDIDO CLIENTE: " + aItemPed[1][21], "")		//"| Nº PEDIDO CLIENTE "	//C5_PEDCLI
