		...
		Else				
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				aadd(aProd,	{Len(aProd)+1,;  
					...
					SB1->B1_UM,;
					(cAliasSD1)->D1_QUANT,;
					//Inicio BOMIX   SANDRO SANTOS 17\01\13
					IIF(!(cAliasSD1)->D1_TIPO$"IP",IIF(!(SubStr((cAliasSD1)->D1_CF,1,1) $ "3"),(cAliasSD1)->D1_TOTAL,(cAliasSD1)->D1_TOTAL),0),;  
					//Fim BOMIX // - (cAliasSD1)->D1_II),0),;// SANDRO SANTOS 17\01\13
					IIF(Empty(SB5->B5_UMDIPI),SB1->B1_UM,SB5->B5_UMDIPI),;                                                           
					...
					//Inicio BOMIX					
				   	IIF(!(cAliasSD1)->D1_TIPO$"IP",IIF(!(SubStr((cAliasSD1)->D1_CF,1,1) $ "3"),(cAliasSD1)->D1_VUNIT,((cAliasSD1)->D1_VUNIT)),0),; 	//BOMIX //-((cAliasSD1)->D1_II/(cAliasSD1)->D1_QUANT)),0),; //Sandro Santos 17\01\2013 ajuste para NFE de IMPORTACAO 
				   	//Fim BOMIX
				   	IIF(SB1->(FieldPos("B1_CODSIMP"))<>0,SB1->B1_CODSIMP,""),; //codigo ANP do combustivel
					IIF(SB1->(FieldPos("B1_CODIF"))<>0,SB1->B1_CODIF,""),; //CODIF  
...
If nValII > 0
	cString += ("Valor total do Imposto de ImportaÁao : R$ " + ConvType(nValII,15,2)+ " ")
Endif
