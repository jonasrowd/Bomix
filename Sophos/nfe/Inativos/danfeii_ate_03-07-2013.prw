...
#DEFINE MAXVALORC  016                                                // Máximo de caracteres por linha de valores numéricos
...
oDanfe:Say(090,001, "Identificação do emitente",oFont10N:oFont)
nLinCalc	:=	100
cStrAux		:=	AllTrim(NoChar(oEmitente:_xNome:Text,lConverte))
nForTo		:=	Len(cStrAux)/100
nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
For nX := 1 To nForTo
	oDanfe:Say(nLinCalc,001,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*50)+1),50), oFont10N:oFont )
	nLinCalc+=07
Next nX
...
If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
	...
	For nX := 1 To nForTo
		...
		nLinCalc+=07
	Next nX
	oDanfe:Say(nLinCalc,001, oEmitente:_EnderEmit:_xMun:Text+"/"+oEmitente:_EnderEmit:_UF:Text,oFont08N:oFont)
	nLinCalc+=07
	oDanfe:Say(nLinCalc,001, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont)
	nLinCalc+=07
	oDanfe:Say(nLinCalc,001, "nfe@bomix.com.br",oFont08N:oFont)
Else
	oDanfe:Say(nLinCalc,001, NoChar(oEmitente:_EnderEmit:_xBairro:Text,lConverte)+" Cep:"+TransForm(IIF(Type("oEmitente:_EnderEmit:_Cep")=="U","",oEmitente:_EnderEmit:_Cep:Text),"@r 99999-999"),oFont08N:oFont)
	nLinCalc+=07
	oDanfe:Say(nLinCalc,001, oEmitente:_EnderEmit:_xMun:Text+"/"+oEmitente:_EnderEmit:_UF:Text,oFont08N:oFont)
	nLinCalc+=07
	oDanfe:Say(nLinCalc,001, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont) 
...
If nfolha==1
	If lMv_Logod
		oDanfe:SayBitmap(044,001,cLogoD,110,035)
	Else
		oDanfe:SayBitmap(044,001,cLogo,110,035)
	EndIF
Endif
