...
//	Local Cabec1         := "Fornecedor              Observação                                      Num.        Pc       Emissão    Vencimento            Valor         "
	Local Cabec1         := "Fornecedor                                                              Num.        Pc       Emissão    Vencimento            Valor         "
	Local Cabec2         := ""
	Local imprime        := .T.
	Local aOrd 			 := {}
       		...
       		*/
       		...
       		For i:=1 To Len(a_Parcelas)
           		If (a_Parcelas[i][1] >= MV_PAR05) .And. (a_Parcelas[i][1] <= MV_PAR06)
	            	...
	            	AADD( a_Titulos, {QRY->FORNECE, QRY->OBS, QRY->NOTA, QRY->PEDIDO, QRY->EMISSAO, Dtos(a_Parcelas[i][1]), a_Parcelas[i][2], QRY->TIPO})
           		Endif
         	Next
         Else
...
//		   		@nLin,024 PSAY PADR(MemoLine(AllTrim(a_Titulos[i][2]),45,1), 45)
		   		@nLin,072 PSAY PADR(a_Titulos[i][3], 9)
