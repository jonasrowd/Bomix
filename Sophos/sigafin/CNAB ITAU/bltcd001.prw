...
±±³Implementação ³ Beatriz-TBA087 - Abril/2009 			          ³±± 
±±³				 ³ Incluir Banco CEF                      ³±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
LOCAL aBolText     := {"Após o vencimento cobrar multa de R$ "                ,;
					   " Protestar Após 03 (três) dias de atraso"}
...
/*
    aDadosBanco  := {SA6->A6_COD                                            						,;  // [1]Numero do Banco
                     ...
                     SUBSTR(xNumCon,Len(AllTrim(xNumCon)),1)                 						,; 	// [5]Dígito da conta corrente
...
ElseIf 	c_Banco = "104" // CEF
        ...
        oPrint:SayBitmap( nRow1+0045,000,cBitMap,350,100 )         //zerada a margem esquerda, para atender a distância de 5mm exigida pela CEF - Bia
...
If 	c_Banco = "104" // CEF
	oPrint:Line (nRow1+0150,000,nRow1+0150,2300) //zerada a margem esquerda, para atender a distância de 5mm exigida pela CEF - Bia
Else
...
//Box 2
/*******/
If 	c_Banco = "104" //zerada a margem esquerda, para atender a distância de 5mm exigida pela CEF - Bia
...
//Box 3
/*******/
If 	c_Banco = "104" // CEF //zerada a margem esquerda, para atender a distância de 5mm exigida pela CEF - Bia
...
/*******/
If 	c_Banco = "104" // CEF //zerada a margem esquerda, para atender a distância de 5mm exigida pela CEF - Bia
	oPrint:Say  (nRow1+0400,000,"Recebi(emos) o bloqueto/título",oFont10)
	oPrint:Say  (nRow1+0450,000,"com as características acima.",oFont10)
Else
	oPrint:Say  (nRow1+0400,100,"Recebi(emos) o bloqueto/título",oFont10)
	oPrint:Say  (nRow1+0450,100,"com as características acima.",oFont10)
Endif
...
ElseIf 	c_Banco = "104" // CEF   //zerada a margem esquerda, para atender a distância de 5mm exigida pela CEF - Bia 
...
//If 		c_Banco = "001" // BANCO DO BRASIL
//   		oPrint:Say  (nRow2+0750,100 ,"PAGÁVEL EM QUALQUER BANCO ATÉ O VENCIMENTO",oFont10)
If 		c_Banco = "237" // BRADESCO
		oPrint:Say  (nRow2+0710,100 ,"Local de Pagamento",oFont8)
   		oPrint:Say  (nRow2+0750,100 ,"PAGÁVEL PREFERENCIALMENTE NAS AGÊNCIAS BRADESCO OU BANCO POSTAL",oFont10)
//ElseIf 	c_Banco = "004" // BNB
//   		oPrint:Say  (nRow2+0750,100 ,"PAGÁVEL EM QUALQUER BANCO ATÉ O VENCIMENTO",oFont10)
ElseIf 	c_Banco = "341" // ITAU
		oPrint:Say  (nRow2+0710,100 ,"Local de Pagamento",oFont8)
   		oPrint:Say  (nRow2+0750,100 ,"ATÉ O VENCIMENTO, PREFERENCIALMENTE NO ITAÚ, APÓS SOMENTE NO ITAÚ",oFont10)                  
ElseIf 	c_Banco = "104" // CEF //zerada a margem esquerda, para atender a distância de 5mm exigida pela CEF - Bia
		oPrint:Say  (nRow2+0710,000 ,"Local de Pagamento",oFont8)
   		oPrint:Say  (nRow2+0750,000 ,"PAGÁVEL EM QUALQUER BANCO ATÉ O VENCIMENTO, APÓS NA CAIXA ECONÔMICA FEDERAL",oFont10)                  
Else   		
		oPrint:Say  (nRow2+0710,100 ,"Local de Pagamento",oFont8)
   		oPrint:Say  (nRow2+0750,100 ,"PAGÁVEL EM QUALQUER BANCO ATÉ O VENCIMENTO",oFont10)
Endif
...
//Box 5
/*******/
If 	aDadosBanco[1] = "104" // CEF //zerada a margem esquerda, para atender a distância de 5mm exigida pela CEF - Bia
...
If 	aDadosBanco[1] = "104" // CEF //zerada a margem esquerda, para atender a distância de 5mm exigida pela CEF - Bia
	oPrint:Say  (nRow2+1050,000 ,"Instruções (Todas informações deste bloqueto são de exclusiva responsabilidade do cedente)",oFont8)
	oPrint:Say  (nRow2+1150,000 ,"Juros por dia de atraso = R$ "+ AllTrim(Transform(NOROUND((_LiqJ/30)),"@E 99,999.99")),oFont10)
	oPrint:Say  (nRow2+1200,000 ,"Protestar no 5 dia útil após o vencimento",oFont10)
...
Else
	...
	oPrint:Say  (nRow2+1200,100 ,"Protestar no 5 dia útil após o vencimento",oFont10)
...
Endif	
//If c_Banco = "001" // BANCO DO BRASIL
//   oPrint:Say  (nRow3+2040,100 ,"PAGÁVEL EM QUALQUER BANCO ATÉ O VENCIMENTO",oFont10)
If 		c_Banco = "237" // BRADESCO
   		oPrint:Say  (nRow3+2040,100 ,"PAGÁVEL PREFERENCIALMENTE NAS AGÊNCIAS BRADESCO OU BANCO POSTAL",oFont10)
//ElseIf c_Banco = "004" // BNB
//   oPrint:Say  (nRow3+2040,100 ,"PAGÁVEL EM QUALQUER BANCO ATÉ O VENCIMENTO",oFont10)
ElseIf 	c_Banco = "341" // ITAU
   		oPrint:Say  (nRow3+2040,100 ,"ATÉ O VENCIMENTO, PREFERENCIALMENTE NO ITAÚ, APÓS SOMENTE NO ITAÚ",oFont10)
ElseIf 	c_Banco = "104" // CEF //zerada a margem esquerda, para atender a distância de 5mm exigida pela CEF - Bia
   		oPrint:Say  (nRow3+2040,000 ,"PAGÁVEL EM QUALQUER BANCO ATÉ O VENCIMENTO, APÓS NA CAIXA ECONÔMICA FEDERAL",oFont10)                  
Else   		
		oPrint:Say  (nRow3+2040,100 ,"PAGÁVEL EM QUALQUER BANCO ATÉ O VENCIMENTO",oFont10)
Endif
...
ElseIf 	aDadosBanco[1] = "104" // CEF //zerada a margem esquerda, para atender a distância de 5mm exigida pela CEF - Bia
...
_LiqJ := ((aDadosTit[5]*_nJur)/100)
If 	aDadosBanco[1] = "104" // CEF //zerada a margem esquerda, para atender a distância de 5mm exigida pela CEF - Bia
	oPrint:Say  (nRow3+2340,000 ,"Instruções (Todas informações deste bloqueto são de exclusiva responsabilidade do cedente)",oFont8)
	oPrint:Say  (nRow3+2390,000 ,"Juros por dia de atraso = R$"+ AllTrim(Transform(NOROUND((_LiqJ/30)),"@E 99,999.99")),oFont10)
	oPrint:Say  (nRow3+2440,000 ,"Protestar no 5 dia útil após o vencimento",oFont10)
...
Else
	...
	oPrint:Say  (nRow3+2440,100 ,"Protestar no 5 dia útil após o vencimento",oFont10)
...
If 		c_Banco = "001" // BANCO DO BRASIL
	    /*
    	COMPOSIÇÃO DO NOSSO-NÚMERO DOS BLOQUETOS DE COBRANÇA:             
	    b) Convênio de seis posições:                                     
    	   I      - CCCCCCNNNNN-X;                                        
	
    	c) Convênios de sete posições, numeração superior a 1.000.000 (um milhão):                                                       
	       I      - CCCCCCCNNNNNNNNNN                                     
    	   OBS: Não existe DV - Dígito Verificador - na composição do nosso-número para convênios
        	    de sete posições, superior a 1.000.000.                  
	       OBS:  "C" - Número do convênio atribuído pelo Sistema Cobrança 
    	        "N" - Seqüencial atribuído pelo cliente ou sistema Cobrança 
         
	    */
    	if empty(SE1->E1_NUMBCO)
	       cNosso    :=SubStr(SEE->EE_CODEMP,1,6)+Strzero(Val(SEE->EE_FAXATU),5) // Convênio de seis posições
...
//  	     nResto:=11-nResto
	    ...
	    //Campo Livre
    	cCampoL := cNosso+cAgencia+STRZERO(VAL(cConta),8)+cCart  // Convenio de seis posições
	//    cCampoL := "000000"+cNosso+cCart  // Convenio de sete posições
ElseIf 	c_Banco = "237" // BRADESCO
		If Empty(SE1->E1_NUMBCO)
    	   nSoma := 0
