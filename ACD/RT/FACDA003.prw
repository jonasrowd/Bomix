...
/*/{Protheus.doc} FACDA003
	Verificar pra que serve
	@type function
	@version 12.1.25
	@author jonas.machado
	@since 20/07/2021
	@param c_Servico, character, ver de onde vem
	@return logical, .T.
/*/
User Function FACDA003(c_Servico)
	...
	If Found()
		dbSelectArea("DC1")
		dbSetOrder(1)
		dbSeek(xFilial("DC1") + DC2->DC2_CODUNI)
		If Found()
			If DC1->DC1_FSACD == "1" .Or. Empty(DC1->DC1_FSACD)
				While l_Unitiz == .F.
					...
					If Found()
						...
						If dbSeek(xFilial("DCO") + DC2->DC2_CODUNI + c_Unitiz)
							If DCO->DCO_STATUS $ "13"
								...
								VTClear Screen
								@ 0, 0 VTSAY "Devolucao? " VTGET c_Devol
								VTRead
								VTClear Screen

							ENDIF

							if (Upper(c_Devol) = "S" .and. DCO->DCO_STATUS = "1") .or. DCO->DCO_STATUS = "2"
