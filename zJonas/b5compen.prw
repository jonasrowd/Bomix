...
/*/
//--------------------------------------------------------------------
User Function B5COMPEN( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "B5COMPEN" )
				Else
					MsgStop( "Atualização não Realizada.", "B5COMPEN" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
SX3->(DbSetOrder(1))
If SX3->(DbSeek('SB5'))
   While !Eof() .And. SX3->X3_ARQUIVO == "SB5"
      cOrdem := SX3->X3_ORDEM
      SX3->(dbSkip())
   End   	
   cOrdem := Soma1(cOrdem)
...
aAdd( aSX3, { ;
	{ 'SB5'																	, .T. }, ; //X3_ARQUIVO
	{ cOrdem																	, .T. }, ; //X3_ORDEM
	{ 'B5_COMPEND'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Comp. Ender.'														, .T. }, ; //X3_TITULO
	{ 'Comp. Direc.'														, .T. }, ; //X3_TITSPA
	{ 'Share Addrss'														, .T. }, ; //X3_TITENG
	{ 'Compartilha Endereco?'												, .T. }, ; //X3_DESCRIC
	{ 'Comparte Direccion'													, .T. }, ; //X3_DESCSPA
	{ 'Share Address?'														, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	...
	{ ''																	, .F. }, ; //X3_OBRIGAT
	{ ''																	, .F. }, ; //X3_VLDUSER
	{ '1=Permite;2=Nao Permite'												, .T. }, ; //X3_CBOX
	{ '1=Permite;2=No Permite'												, .T. }, ; //X3_CBOXSPA
	{ '1=Allows it;2=Does not allow it'										, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ ''																	, .F. }, ; //X3_WHEN
	{ ''																	, .T. }, ; //X3_INIBRW
	{ ''																	, .T. }, ; //X3_GRPSXG
