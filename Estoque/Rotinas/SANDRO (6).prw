...
/*
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ºPrograma  ³ANALISE   ºAutor  ³Sandro Santos       º Data ³  15/01/2013 º±±
...
//@ 110,170 SAY cMensNeg  VAR cMensNeg                         SIZE 200,50 PIXEL OF oFolder:aDialogs[1] FONT oBold2 COLOR CLR_HRED
...
//@ 010,420 BUTTON "&Processar"   SIZE 36,16 PIXEL ACTION Processa({||fProcessa()})   Alterado por Sandro Santos em 21/01/13
...
Do Case
   Case SB1->B1_LOCALIZ == "S" .AND. SB1->B1_RASTRO == "L"  
      ...
      IF nSBJ = 0 .AND.  nSBK = 0 .AND. nKAR = nSB8 .AND. nKAR = nSD5 .AND. nKAR = nSDB .AND. nKAR = nSBF .AND. nKAR = nSB2 .AND. nSB8 = nSD5 .AND. nSBF = nSDB .AND. nSB8 = nSBF .AND. nSB2 = nSB8 .AND. nSB2 = nSBF 
         cMensagem := "OK"  
      Endif 
       IF nSb2 < 0 .AND. nSB8 <0                                 // Ajuste para saldo iguais (NEGATIVO) Sandro Santos 
         cMensagem := "SLD NEGATIVO"
      Endif                                    
      IF nSb2 < 0 .AND. nSB8 <0 .AND. nSD5 <0                  // Ajuste para saldo iguais (NEGATIVO) Sandro Santos 
         cMensagem := "SLD NEGATIVO"
      Endif
     IF nSb2 < 0                                             // Ajuste para saldo iguais (NEGATIVO) Sandro Santos 
         cMensagem := "SLD NEGATIVO"
      Endif 
...
ElseIF cMensagem = "SLD NEGATIVO"      // Ajuste para saldo iguais (NEGATIVO) Sandro Santos
   cMensOK   := ""
   cMensErro := "SLD NEGATIVO"
Endif                      
...
For I:= 1 To Len(aDocto)
  ...
  Endif 
Next                              // Usar somente para cliente em processo de ajuste ou implantação SANDRO SANTOS
aSize(aDocto,0)
...
5.7
Correção na validação dos saldos quando estive iguai e negativos
*/