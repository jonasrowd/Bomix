...
User Function FESTA001
	...
	Private l_OPsInt   := .F.
	Private l_AllOPs   := .F.
	Private l_HabOpc1  := .F.	//Flag para habilitar a opção de liberação de OPs intermediárias relacionadas ao produto
	Private l_HabOpc2  := .F.	//Flag para habilitar a opção de liberação de todas as OPs relacionadas ao produto
...
//	oGet1      := TGet():New( 008,028,{|u| IIF(PCOUNT()>0,c_Produto:=u, c_Produto)},oDlg1,076,008,'',{|| f_VldCod()},CLR_BLACK,CLR_WHITE,,,,.T.,"",,,.F.,.F.,,.F.,.F.,"SB1","",,)
	...
	oSay2      := TSay():New( 008,116,{||"Descrição:"},oDlg1,,,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,032,008)
	oGet2      := TGet():New( 008,144,{|u| IIF(PCOUNT()>0,c_Desc:=u, c_Desc)},oDlg1,192,008,'',,CLR_BLACK,CLR_WHITE,,,,.T.,"",,,.F.,.F.,,.F.,.F.,"","",,)

	oGrp1      := TGroup():New( 024,004,156,336,"Ordens de Produção Previstas do Produto",oDlg1,CLR_BLACK,CLR_WHITE,.T.,.F. )
	...
	oCBox2     := TCheckBox():New( 188,012,"Produto pronto para impressão",{|| l_AllOPs},oGrp2,164,008,,{|| l_AllOPs := !l_AllOPs},,,CLR_BLACK,CLR_WHITE,,.T.,"",,{|| l_HabOpc2} )
...
Static Function f_GetOPs
	...
	If QRY->(!Eof())
		While QRY->(!Eof())
			If QRY->C2_QTDPREV > 0					//Se existirem OPs intermediárias previstas deve atualizar o campo C2_FSTPOPI da OP pai
				dbSelectArea("SC2")
				dbGoTop()
				dbSetOrder(9)
