...
Static Function f_OpsApt()
	...
	'<p>A Arte ' + SZ2->Z2_COD + ' - ' + AllTrim(SZ2->Z2_DESC) + ' houve uma tentativa de bloqueio da arte pelo usuário ( ' + AllTrim(UsrRetName(__CUSERID)) + ' ), verifique com o setor o motivo:</p>' +;
	'<table>' +;
	'<tr>' +;
	...
	If bQRY->(!Eof())
		...
		If !Empty(c_To)	
			...
			l_EnviaE = .T.
			ShowHelpDlg(SM0->M0_NOME,;
			{"Existem OPs em produção e não é permitido bloquear a arte. " },5,;
...
Static Function f_OpsAptc()
	...
	Local a_Area     := GetArea()
	Local c_To       := GETMV("FS_EMAILC")	//Destinatário do e-mail
	Local c_Corpo    := ''					//Corpo do e-mail
	Private a_Strut  := {}
	Private a_Campos := {}
	...
	c_Corpo :=  '<p>Prezado(a)</p>' +;
	'<p>Na Arte ' + SZ2->Z2_COD + ' - ' + AllTrim(SZ2->Z2_DESC) + ' foi alterado seu STATUS pelo usuário ( ' + AllTrim(UsrRetName(__CUSERID)) + ' )</p>' +;
	'<table>' +;
	'<tr>' +;
	'<td align="left" style="padding-top: 10px; padding-bottom: 10px;">' +;
	'<table cellspacing="0" border="1" cellpadding="4">' +;
...
/*/{Protheus.doc} RunReport
	...
	@author jonas.machado
	@since 21/07/2021
	@param Cabec1, character, Cabeçalho
	@param Cabec2, character, Cabeçalho
	@param Titulo, variant, Título
	@param nLin, numeric, Número da linha calculada no for
	@return variant, Null
/*/
...
/*/{Protheus.doc} f_Enviar
	...
	@author jonas.machado
	@since 21/07/2021
	@param c_To, character, Destinatário do e-mail
...
Static Function f_Enviar(c_To, c_Corpo)
	...
	*/
	...
	IF M->Z2_BLOQ='2'
		U_TBSENDMAIL(c_To, c_Corpo, 'Notificação de tentativa de bloqueio da Arte ' + AllTrim(SZ2->Z2_DESC), .T.)
	ELSE
		U_TBSENDMAIL(c_To, c_Corpo, 'Alteração de STATUS da ARTE "'+AllTrim(SZ2->Z2_DESC)+ '" para: "'+c_status+'" ', .T.)
	Endif	
	l_EnviaE := .T.
Return
