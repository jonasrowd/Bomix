...
//		oReport:SkipLine()
		...
		oReport:PrintText( 'Aprovação' ,nLinPC, 050 ) //"Comprador" substituído por Aprovação
		oReport:PrintText( 'Fornecedor' ,nLinPC, 430 ) //"Gerencia" substituído por Fornecedor
		oReport:PrintText( 'C. Pagar' ,nLinPC, 850 ) //"Diretoria" substituído por C. Pagar
		oReport:SkipLine()
...
//		oReport:SkipLine()
		...
		oReport:SkipLine()
		nLinPC := oReport:Row()
		oReport:PrintText( "I  - Só aceitaremos a mercadoria se na sua Nota Fiscal constar o número do nosso Pedido de Compras." ,nLinPC, 050 )
		//Fim Bomix  (Sandro Santos 30/01/13)
		
		oReport:SkipLine()
		nLinPC := oReport:Row()
		oReport:PrintText("II - O pagamento da nota fiscal correspondente a esta autorização de fornecimento somente será efetuado após nossa confirmação do recebimento do arquivo XML" ,nLinPC, 050 )
