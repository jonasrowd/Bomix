...
±±³Fun‡…o    ³ FMATR110  ³ Autor ³ Alexandre Inacio Lemes³ Data ³06/09/2006³±±
...
/*/
User Function FMATR110( cAlias, nReg, nOpcx )
...
While !oReport:Cancel() .And. !SC7->(Eof()) .And. SC7->C7_FILIAL == xFilial("SC7") .And. SC7->C7_NUM >= mv_par01 .And. SC7->C7_NUM <= mv_par02
	...
	For nVias := 1 to mv_par09
		...
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		If Empty(cObs02)
			If Len(cObs01) > 160	/*30*/
				cObs := cObs01
				cObs01 := Substr(cObs,1,160/*30*/)
				For nX := 2 To 16
					cVar  := "cObs"+StrZero(nX,2)
					&cVar := Substr(cObs,(160/*30*/*(nX-1))+1,160/*30*/)
				Next nX
			EndIf
		Else
			cObs01:= Substr(cObs01,1,IIf(Len(cObs01)<160/*30*/,Len(cObs01),160/*30*/))
			cObs02:= Substr(cObs02,1,IIf(Len(cObs02)<160/*30*/,Len(cObs01),160/*30*/))
			cObs03:= Substr(cObs03,1,IIf(Len(cObs03)<160/*30*/,Len(cObs01),160/*30*/))
			cObs04:= Substr(cObs04,1,IIf(Len(cObs04)<160/*30*/,Len(cObs01),160/*30*/))
			cObs05:= Substr(cObs05,1,IIf(Len(cObs05)<160/*30*/,Len(cObs01),160/*30*/))
			cObs06:= Substr(cObs06,1,IIf(Len(cObs06)<160/*30*/,Len(cObs01),160/*30*/))
			cObs07:= Substr(cObs07,1,IIf(Len(cObs07)<160/*30*/,Len(cObs01),160/*30*/))
			cObs08:= Substr(cObs08,1,IIf(Len(cObs08)<160/*30*/,Len(cObs01),160/*30*/))
			cObs09:= Substr(cObs09,1,IIf(Len(cObs09)<160/*30*/,Len(cObs01),160/*30*/))
			cObs10:= Substr(cObs10,1,IIf(Len(cObs10)<160/*30*/,Len(cObs01),160/*30*/))
			cObs11:= Substr(cObs11,1,IIf(Len(cObs11)<160/*30*/,Len(cObs01),160/*30*/))
			cObs12:= Substr(cObs12,1,IIf(Len(cObs12)<160/*30*/,Len(cObs01),160/*30*/))
			cObs13:= Substr(cObs13,1,IIf(Len(cObs13)<160/*30*/,Len(cObs01),160/*30*/))
			cObs14:= Substr(cObs14,1,IIf(Len(cObs14)<160/*30*/,Len(cObs01),160/*30*/))
			cObs15:= Substr(cObs15,1,IIf(Len(cObs15)<160/*30*/,Len(cObs01),160/*30*/))
			cObs16:= Substr(cObs16,1,IIf(Len(cObs16)<160/*30*/,Len(cObs01),160/*30*/))
		EndIf
		...
		nLinPC := oReport:Row()
		oReport:PrintText(STR0052+" "+Substr(cComprador,1,60),nLinPC,050 ) 	//"Comprador Responsavel :" //"BLQ:Bloqueado"
		...
		oReport:PrintText(STR0053+" "+If( Len(cAlter) > 0 , Substr(cAlter,001,080) , " " ),,050 ) //"Compradores Alternativos :"
		...
		oReport:PrintText(If( Len(cAlter) > 0 , Substr(cAlter,081,080) , " " ),,050 ) //"Compradores Alternativos :"
		...
		oReport:PrintText(STR0054+" "+If( Len(cAprov) > 0 , Substr(cAprov,001,080) , " " ),,050 ) //"Aprovador(es) :"		
        /*
