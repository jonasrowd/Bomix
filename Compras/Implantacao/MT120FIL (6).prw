...
aItems:={"Pedido Pendente e Pedido Parcialmente Atendido","Pedido Pendente","Pedido Parcialmente Atendido","Pedido Atendido","Pedido Bloqueado",;
         "Elim. Residuo","Utilizado em Pre Documento de Entrada","Autorização de Entrega",;
		 "Integração Modulo Gestão de Contratos","Aguardando Confirmação do MarketPlace"} 
	...
	oBtnCancel  := TButton():New(100,070,"NÃO FILTRA (F5)",oDlg,{||NAO_FILTRA()},50,20,,,,.T.,,"",,,,.F.)
...
Do Case 
 ...
 case  c_Filtro == "Autorização de Entrega"
	 cRet := "C7_TIPO!=nTipoPed"+c_Filial

 case  c_Filtro == "Integração Modulo Gestão de Contratos"
	 cRet := "!Empty(C7_CONTRA).And.Empty(C7_RESIDUO)"+c_Filial

 case  c_Filtro == "Aguardando Confirmação do MarketPlace'	
	 cRet := ""+c_Filial
 OtherWise
	 cRet := ""
EndCase	 
