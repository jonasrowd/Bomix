...
User Function FGPER001()
	...
	Private oArial13N	:=	TFont():New("Arial",,11,,.T.,,,,,.F.,.F.)
	Private oArial14N	:=	TFont():New("Arial Narrow",,14,,.T.,,,,,.F.,.F.)
	Private oArial11N	:=	TFont():New("Arial",,10,,.F.,,,,,.F.,.F.)
	...
	Private oPrinter  	:=	tAvPrinter():New("Abono Pecuniario")
...
Static Function printPage()
	...
	WHILE	QRY->(!EOF())
		...
		d_final	:= MV_PAR04
		c_Nome  	:= QRY->RA_NOME
		c_Cargo 	:= QRY->RA_CODFUNC + " - "+ ALLTRIM(QRY->RJ_DESC)
		...
		oPrinter:Say(0071,0534,"Solicitação de Abono de Férias",oArial30,,8421504)
		...
		oPrinter:Say(0800,0360,"Em cumprimento ao disposto no parágrafo 1. do Artigo 143 do Decreto_Lei número 1535 de 13 de Abril de ",oArial11N,,0)
   	    oPrinter:Say(0850,0360,"1977, venho pelo presente requerer o ABONO PECUNIÁRIO de 1/3 (um terço) das férias referente período ",oArial11N,,0)
		oPrinter:Say(0900,0360,"aquisitivo de "+ c_aquisitivo ,oArial11N,,0)
		//oPrinter:Say(0857,0758," "+ d_Per1,oArial7N,,0)
		//oPrinter:Say(0855,1080," "+ d_Per2,oArial7N,,0)
		oPrinter:Say(1104,0360," "+ c_Cidade + ", " + d_Emissao ,oArial11N,,0)
		oPrinter:Box(1340,0360,1340,1101)
		oPrinter:Box(1340,1215,1340,1976)
		oPrinter:Say(1348,0360," " + c_Empresa ,oArial11N,,0)
		oPrinter:Say(1348,1350," " + c_Nome ,oArial11N,,0)
		oPrinter:Box(0066,0074,0179,2352)
		oPrinter:EndPage()
		oPrinter:Box(0066,0074,0179,2352)

		//finaliza página aqui
		QRY->(DBSKIP())
...
static function f_Consulta()
	...
	cQuery := "SELECT RA_MAT, RA_NOME, RA_CIC, RA_CODFUNC, RJ_DESC "
