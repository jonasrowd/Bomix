		...
		If SRA->RA_DEMISSA >= MV_PAR01 .AND. SRA->RA_DEMISSA <= MV_PAR02    
			...
			IF Alltrim(SRA->RA_SITFOLH) =="D"
				...
				if (MOD(n_Count, 56) = 0)
					...
					n_Row := 0442
					n_pag ++
					oPrinter:Say(3357,0084,"Total:"+ cValToChar(n_Count)	,o10N,,0)//roda pé
					oPrinter:Say(3357,1884,'Pág.:' + cValToChar(n_pag),o10N,,0)
					n_Count:= 0
					oPrinter:EndPage()
					//ELSEIF n_Count < 56
					//oPrinter:Say(3357,0084,"Total:"+ cValToChar(n_Count)	,o10N,,0)
					//oPrinter:Say(3357,1884,'Pág.:' + cValToChar(n_pag+1),o10N,,0)
				endIf
			endif  
