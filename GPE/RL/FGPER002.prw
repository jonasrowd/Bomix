...
Static Function printPage()
	...
	IF  dbSeek(xFilial("SRA")+MV_PAR01)
	   ...
	   // c_desc :=   "1. O(A) EMPREGADO(A), exercerá a função de "+ MDESC +", submetido a controle de ponto em ",oArial10,,0)"
	   ...
	   n_Lin = n_Lin+50
		...
		n_Lin = n_Lin+100	        
    	oPrinter:Say(n_lin, n_col,"1.O(A) EMPREGADO(A),exercera a função de :"+ MDESC +",com a remuneração mensal de "+Transform(c_Salario,"@E 9,999,999.99"),oArial10,,0)
	    n_Lin = n_Lin+50  
		oPrinter:Say(n_lin,n_col, ALLTRIM(c_SalExt)+"*****,por mês.",oArial10,,0)
        n_Lin = n_Lin+100      
		...
		n_lin:= n_lin+50
		oPrinter:Say(n_lin,n_col,"refeição, podendo a EMPREGADORA, alterá-lo de acordo com as necessidades do serviço.",oArial10,,0)
		n_lin:= n_lin+100
        oPrinter:Say(n_lin,n_col,"5. Qualquer gratificação, prêmio, beneficio, etc., que EMPREGADORA, vier a conceder por  liberalidade, não ",oArial10,,0)
		n_lin:= n_lin+50
		oPrinter:Say( n_lin,n_col,"serão incorporados ao salário para os efeitos legais não se considerando renovação contratual a concessão ",oArial10,,0)
		n_lin:= n_lin+50
		oPrinter:Say(n_lin,n_col,",habitual ou não,de tais gratificações.",oArial10,,0)
		n_lin:= n_lin+100    
		oPrinter:Say(n_lin,n_col,"6. A prática de qualquer ato prejudicial à EMPREGADORA,tais como: indisciplina, insubordinação desídia,etc.",oArial10,,0)
		n_lin:= n_lin+50
		oPrinter:Say(n_lin,n_col,",implica na rescisão automática deste Contrato.",oArial10,,0)
		n_lin:= n_lin+100
		...
		n_lin:= n_lin+50 
		oPrinter:Say( n_lin,n_col,"to da importância correspondente ao  prejuízo,  conforme  previsto no parágrafo 1º  do artigo 462 da CLT,",oArial10,,0)
		n_lin:= n_lin+50
		oPrinter:Say( n_lin,n_col,"concordando o empregado, desde já, em ressarcir o  já, em ressarcir o dano causado.",oArial10,,0)
		n_lin:= n_lin+100
		...
		n_lin:= n_lin+50
		oPrinter:Say(n_lin,n_col,"compensação de horas e revezamento de horário,  inclusive em  período noturno.",oArial10,,0)
		n_lin:= n_lin+100         
		
        oPrinter:Say(n_lin,n_col,"9.Toda e qualquer informação que o(a) empregado venha a ter conhecimento em decorrencia do contrato de trabalho",oArial10,,0)
        n_lin:= n_lin+50
        oPrinter:Say(n_lin,n_col,"é de carater sigiloso e não poderá ser divulgada pelo(a) empregado(a), sob qualquer hipotese responsabilizando-se",oArial10,,0)
        n_lin:= n_lin+50
        oPrinter:Say(n_lin,n_col,"o(a) empregado civil e criminalmente, a qualquer tempo pela violação do sigilo ou uso das informações em disconfor- ",oArial10,,0)                
        n_lin:= n_lin+50
        oPrinter:Say(n_lin,n_col,"midade com o presente contrato",oArial10,,0)                
        
		n_lin:= n_lin+100                 
		oPrinter:Say(n_lin,n_col,"10.O prazo deste Contrato é de 30 (trinta) dias, com inicio em "+alltrim(PADR(d_data,80))+" podendo ser prorrogado",oArial10,,0)
		n_lin:= n_lin+50
		oPrinter:Say(n_lin,n_col,"ate 90(noventa) dias.",oArial10,,0)
		n_lin:= n_lin+100
		oPrinter:Say(n_lin,n_col,"11.Permanecendo o (a) EMPREGADO(A) a serviço da EMPREGADORA após o término da experiência,o Contrato",oArial10,,0)
		n_lin:= n_lin+50
		oPrinter:Say(n_lin,n_col,"passará a viger por prazo indeterminado,com plena vigência das demais Cláusulas aqui pactuadas.",oArial10,,0)
		n_lin:= n_lin+100   
		...
		n_lin:= n_lin+50
		oPrinter:Say(n_lin,n_col,"vias , ficando  a primeira  em  poder da EMPREGADORA e a segunda como(a) EMPREGADO(A), que dela dará o",oArial10,,0)
		n_lin:= n_lin+50
