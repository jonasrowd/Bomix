...
//	Local Cabec2         := "Cliente/Fornecedor                                           Emissão   Num.        Pc       Venc Real  Vencimento            Valor         "
	Local Cabec2         := "Cliente/Fornecedor                Emissão   Num.        Pc       Venc Real  Vencimento        Receber          Pagar          Saldo"	
	Local imprime        := .T.
	Local aOrd 			 := {}
       		...
       		*/
       		...
       		For i:=1 To Len(a_Parcelas)
           		If (a_Parcelas[i][1] >= MV_PAR07) .And. (a_Parcelas[i][1] <= MV_PAR08)
	            	...
	            	AADD( a_Titulos, {QRY->FORNECE, QRY->OBS, QRY->NOTA, QRY->PEDIDO, QRY->EMISSAO, Dtos(a_Parcelas[i][1]), a_Parcelas[i][2], "Pagar", Dtos(a_Parcelas[i][1])})
           		Endif
         	Next
...
//		       	@nLin,000 PSAY PADR(a_Titulos[i][1], 20)
		       	@nLin,000 PSAY PADR(a_Titulos[i][1], 30)
//		   		@nLin,024 PSAY PADR(MemoLine(AllTrim(a_Titulos[i][2]),45,1), 45)

		       	@nLin,034 PSAY PADR(Stod(a_Titulos[i][5]), 10)
		       	
		   		@nLin,044 PSAY PADR(a_Titulos[i][3], 9)
		       	@nLin,056 PSAY PADR(a_Titulos[i][4], 6)
		       	@nLin,065 PSAY PADR(Stod(a_Titulos[i][9]), 10)
	    		@nLin,076 PSAY PADR(Stod(a_Titulos[i][6]), 10)
				If AllTrim(c_Tipo) == 'Pagar'
		    		@nLin,102 PSAY Transform(a_Titulos[i][7],"@E 999,999,999.99")
		  		Else
		    		@nLin,087 PSAY Transform(a_Titulos[i][7],"@E 999,999,999.99")
		  		Endif
	    		...
	    		@nLin,117 PSAY Transform(n_Saldo,"@E 999,999,999.99")	   
...
//	  	@nLin,117 PSAY Transform(n_TotDiaRc,"@E 999,999,999.99")
	  	@nLin,087 PSAY Transform(n_TotDiaRc,"@E 999,999,999.99")
	  	nLin := nLin + 1 // Avanca a linha de impressao
...
//  	@nLin,117 PSAY Transform(n_Total,"@E 999,999,999.99")
  	@nLin,102 PSAY Transform(n_Total,"@E 999,999,999.99")
  	nLin := nLin + 1 // Avanca a linha de impressao
  	@nLin,000 PSAY "Total a Receber no Período - "
//  	@nLin,117 PSAY Transform(n_TotalRec,"@E 999,999,999.99")
  	@nLin,087 PSAY Transform(n_TotalRec,"@E 999,999,999.99")
  	nLin := nLin + 1 // Avanca a linha de impressao
...
//  	c_Qry += " (SELECT A2_NOME FORNECE, C7_OBS OBS, '' NOTA, C7_NUM PEDIDO, C7_EMISSAO EMISSAO, '' VENC, SUM(C7_TOTAL)+SUM(C7_VALIPI) AS VALOR, C7_COND CONDICAO, 'Previstos' TIPO" + chr(13)
  	...
  	c_Qry += " JOIN " + RetSqlName("SA2") + " SA2 ON A2_COD = C7_FORNECE AND A2_LOJA = C7_LOJA AND (A2_COD BETWEEN '" + MV_PAR03 + "' AND '" + MV_PAR04 + "') AND SA2.D_E_L_E_T_<>'*' " + chr(13)
  	c_Qry += " WHERE C7_CONAPRO = 'L' AND C7_QUJE < C7_QUANT AND (C7_EMISSAO BETWEEN '" + Dtos(MV_PAR05) + "' AND '" + Dtos(MV_PAR06) + "') AND SC7.D_E_L_E_T_<>'*' " + chr(13)
 	...
 	c_Qry += " SELECT CASE WHEN E2_ORIGEM='MATA460' THEN A1_NOME ELSE A2_NOME END, E2_HIST, E2_NUM, '', E2_EMISSAO, E2_VENCREA, E2_VALOR, '', 'Pagar' " + chr(13)
  	c_Qry += " FROM " + RetSqlName("SE2") + " SE2 " + chr(13)
  	c_Qry += " JOIN " + RetSqlName("SA2") + " SA2 ON A2_COD = E2_FORNECE AND A2_LOJA = E2_LOJA AND SA2.D_E_L_E_T_<>'*' " + chr(13)
  	c_Qry += " JOIN " + RetSqlName("SA1") + " SA1 ON A1_COD = E2_FORNECE AND A1_LOJA = E2_LOJA AND SA1.D_E_L_E_T_<>'*' " + chr(13)
//  	c_Qry += " WHERE E2_TIPO <> 'PA' AND E2_MULTNAT <> '1' AND E2_SALDO <> 0 AND SE2.D_E_L_E_T_<>'*' " + chr(13)
  	...
  	c_Qry += "   	UNION ALL " + chr(13)
 	c_Qry += " SELECT A1_NOME, E1_HIST, E1_NUM, '', E1_EMISSAO, E1_VENCREA, E1_VALOR, '', 'Receber' " + chr(13)
  	c_Qry += " FROM " + RetSqlName("SE1") + " SE1 " + chr(13)
  	c_Qry += " JOIN " + RetSqlName("SA1") + " SA1 ON A1_COD = E1_CLIENTE AND A1_LOJA = E1_LOJA AND SA1.D_E_L_E_T_<>'*' " + chr(13)
