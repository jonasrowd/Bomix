...
±±ºPrograma  ³FESTA006  ºAutor  ³                    º Data ³Julho/2011   º±±
...
User Function FESTA006()
	Local c_Texto  := "Esta rotina tem a finalidade de importar dados para tabela SB7 - Inventário, a partir do arquivo csv selecionado  pelo usuário."
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³f_ImportaDadosºAutor  ³                º Data ³ Julho/2011  º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
Static Function f_Importa()
	...
	Private c_Linha    := ""
	Private c_Filial   := xFilial("SB7") // Filial do Inventário
	...
	IF AVISO(SM0->M0_NOMECOM,"Esta rotina irá importar os dados do arquivo texto para a tabela SB7 - Inventário. Deseja realmente continuar?",{"Sim","Não"},2,"Atenção") == 1
		CriaPerg(c_Perg)
		...
		WHILE !FT_FEOF()
			...
			c_Desc     := ""
			c_Local    := a_Invent[2]
			n_Quant    := Val(StrTran(a_Invent[3], ",", "."))
			c_Lote     := IIF(Len(a_Invent) >= 4, a_Invent[4], "")
			c_DtValid  := IIF(Len(a_Invent) >= 5, a_Invent[5], "")
			c_Endereco := IIF(Len(a_Invent) >= 6, Padr(a_Invent[6], TamSX3("B7_LOCALIZ")[1]), "")
			n_PrValid  := 0
			c_Obs      := ""
		...
		FT_FUSE()
		AVISO(SM0->M0_NOME,"O programa processou todo o arquivo com " + ALLTRIM(STR(n_QtdInc+n_QtdUpd+n_QtdErr)) + " registros. Foram importados " + ALLTRIM(STR(n_QtdInc)) + " registros, " + ALLTRIM(STR(n_QtdUpd)) + " registros foram atualizados e " + ALLTRIM(STR(n_QtdErr)) + " registros não foram importados.",{"OK"},2,"Aviso")
		...
		Aadd(a_Campos,{"TB_LOCALIZ" ,,'Endereço'  	,'@!'})
...
Static Function f_ExpLog()
	Local c_Destino := FCREATE("C:\TEMP\LOG_IMPORTAÇÃO_INVENTÁRIO_" + AllTrim(c_Doc) + ".CSV")
	Local c_Linha := ""
